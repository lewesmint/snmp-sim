...
def test_schema_loading() -> None:
    ...
    for idx, row in enumerate(rows):
        ...
        if idx == 0:
            assert decoded == b'\x00\x11\x22\x33\x44\x55', f"Row 0 MAC mismatch"
        elif idx == 1:
            assert decoded == b'\xAA\xBB\xCC\xDD\xEE\xFF', f"Row 1 MAC mismatch"
        elif idx == 2:
            assert decoded == b'\x00\x00\x00\x00\x00\x00', f"Row 2 MAC mismatch"
