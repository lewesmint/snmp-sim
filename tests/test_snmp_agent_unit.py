...
def test_setup_signal_handlers_registers_signals(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_run_aborts_when_type_registry_validation_fails(monkeypatch: pytest.MonkeyPatch, caplog: pytest.LogCaptureFixture) -> None:
    agent = SNMPAgent(preloaded_model={})
...
def test_setup_snmpEngine_loads_compiled_modules(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, caplog: pytest.LogCaptureFixture) -> None:
    ...
    monkeypatch.setitem(sys.modules, 'pysnmp.entity.engine', types.SimpleNamespace(SnmpEngine=FakeEngine))
    ...
    monkeypatch.setitem(sys.modules, 'pysnmp.carrier.asyncio.dispatch', types.SimpleNamespace(AsyncioDispatcher=FakeDispatcher))
    ...
    class FakeMibBuilder:
        ...
        def import_symbols(self, *args: Any, **kwargs: Any) -> tuple[str, ...]:
            return ('MibScalar', 'MibScalarInstance', 'MibTable', 'MibTableRow', 'MibTableColumn')
    ...
    monkeypatch.setitem(
        ...
        types.SimpleNamespace(SnmpContext=lambda eng: types.SimpleNamespace(get_mib_instrum=lambda: types.SimpleNamespace(get_mib_builder=fake_get_mib_builder)))
    )

    monkeypatch.setitem(sys.modules, 'pysnmp.smi.builder', types.SimpleNamespace(DirMibSource=lambda p: f"DirMibSource({p})"))
...
def test_setup_snmpEngine_handles_no_compiled_modules(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, caplog: pytest.LogCaptureFixture) -> None:
    ...
    monkeypatch.setitem(sys.modules, 'pysnmp.entity.engine', types.SimpleNamespace(SnmpEngine=FakeEngine2))
    ...
    monkeypatch.setitem(sys.modules, 'pysnmp.carrier.asyncio.dispatch', types.SimpleNamespace(AsyncioDispatcher=FakeDispatcher2))
    ...
    monkeypatch.setitem(
        sys.modules,
        'pysnmp.entity.rfc3413.context',
        types.SimpleNamespace(SnmpContext=lambda eng: types.SimpleNamespace(get_mib_instrum=lambda: types.SimpleNamespace(get_mib_builder=lambda: FakeMibBuilder()))),
    )
    monkeypatch.setitem(sys.modules, 'pysnmp.smi.builder', types.SimpleNamespace(DirMibSource=lambda p: f"DirMibSource({p})"))
...
def test_setup_transport_adds_transport(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setitem(sys.modules, 'pysnmp.carrier.asyncio.dgram', types.SimpleNamespace(udp=FakeUdp))
    ...
    try:
        ...
        monkeypatch.setitem(sys.modules, 'pysnmp.entity.config', types.SimpleNamespace(add_transport=fake_add_transport, SNMP_UDP_DOMAIN='udp'))
...
def test_setup_responders_registers_responders(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    engine_obj = types.SimpleNamespace(message_dispatcher=types.SimpleNamespace(register_context_engine_id=lambda *_a, **_k: None))
