...
def test_links_helpers_format_and_parse_endpoints() -> None:
    """Test case for test_links_helpers_format_and_parse_endpoints."""
    gui = _mk_gui()
...
def test_varbind_detection_index_and_sysuptime() -> None:
    """Test case for test_varbind_detection_index_and_sysuptime."""
    gui = _mk_gui()
...
def test_oid_metadata_and_table_oid_resolution_paths() -> None:
    """Test case for test_oid_metadata_and_table_oid_resolution_paths."""
    gui = _mk_gui()
    gui.oid_metadata = {
        "1.3.6.1.2.1.1.5": {
            "mib": "SNMPv2-MIB",
...
def test_enum_and_selected_info_helpers() -> None:
    """Test case for test_enum_and_selected_info_helpers."""
    gui = _mk_gui()
    enums = {"up": 1, "down": 2}
...
def test_index_encode_decode_helpers_with_ipaddress() -> None:
    """Test case for test_index_encode_decode_helpers_with_ipaddress."""
    gui = _mk_gui()
...
def test_decompose_table_oid_returns_table_column_and_instance() -> None:
    """Test case for test_decompose_table_oid_returns_table_column_and_instance."""
    gui = _mk_gui()
    gui.table_schemas = {
        "ifTable": {
            "oid": [1, 3, 6, 1, 2, 1, 2, 2],
...
def test_collect_trap_overrides_skips_index_and_sysuptime() -> None:
    """Test case for test_collect_trap_overrides_skips_index_and_sysuptime."""
    gui = _mk_gui()
    gui.oid_rows = [
        {
...
def test_resolve_apply_overrides_and_payload(monkeypatch: Any) -> None:
    """Test case for test_resolve_apply_overrides_and_payload."""
    gui = _mk_gui()
    logs: list[tuple[str, str]] = []
    monkeypatch.setattr(gui, "_log", lambda message, level="INFO": logs.append((level, message)))

    def resolve_oid(oid_str: str) -> str | None:
        return {
            "ok": "1.3.6.1.2.1.1.5.0",
            "ok2": "1.3.6.1.2.1.1.6.0",
        }.get(oid_str)

    monkeypatch.setattr(gui, "_resolve_oid_str_to_actual_oid", resolve_oid)
    ...
    monkeypatch.setattr("ui.snmp_gui.requests.post", fake_post)
...
def test_log_send_helpers_write_text_widget() -> None:
    ...
    assert len(log_text.insert_calls) == 2
    combined = "\n".join(text for _, text in log_text.insert_calls)
    ...
    assert log_text.see_calls == ["end", "end"]


def test_trap_index_helpers_and_interface_indices(monkeypatch: Any) -> None:
    """Test case for test_trap_index_helpers_and_interface_indices."""
    gui = _mk_gui()
    ...
    def fake_get(url: str, params: dict[str, Any] | None = None, timeout: int = 0) -> _JsonResp:
        _ = (params, timeout)
        if url.endswith("/table-schema"):
            return _JsonResp(200, {"instances": ["3", "1", "2"]})
        return _JsonResp(500, {})

    monkeypatch.setattr("ui.snmp_gui.requests.get", fake_get)
...
def test_interface_indices_fallback_to_ifnumber(monkeypatch: Any) -> None:
    """Test case for test_interface_indices_fallback_to_ifnumber."""
    gui = _mk_gui()
...
def test_interface_indices_probe_fallback(monkeypatch: Any) -> None:
    """Test case for test_interface_indices_probe_fallback."""
    gui = _mk_gui()
