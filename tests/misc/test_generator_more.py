...
def test_detect_inherited_indexes_through_extract(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    symbols: dict[str, object] = {"MyEntry": EntryWithIndex(), "otherCol": Column([1, 3, 6, 9])}
...
def test_get_default_value_from_type_info_raises(monkeypatch: pytest.MonkeyPatch) -> None:
    g = BehaviourGenerator(output_dir=".", load_default_plugins=False)
...
def test_generate_creates_default_table_row_with_index_extraction(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
...
def test_generate_handles_dir_mib_source_exception(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
...
def test_extract_mib_info_handles_non_dict_mib_symbols(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_extract_mib_info_handles_symbol_without_getname(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_get_default_value_from_type_info_raises_on_no_plugin(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_get_default_value_legacy_cases(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert g._get_default_value("DisplayString", "sysDescr") == "Simple Python SNMP Agent - Demo System"
...
def test_generate_force_regenerate_removes_existing_file(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
...
def test_extract_mib_info_add_mib_sources_exception_handling(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_extract_mib_info_getIndexNames_exception(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_extract_mib_info_symbol_info_missing_type(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_write_schema_debug_logging(monkeypatch: pytest.MonkeyPatch, caplog: pytest.LogCaptureFixture, tmp_path: Path) -> None:
    ...
    import app.generator as genmod
    monkeypatch.setattr(genmod, "builder", types.SimpleNamespace(MibBuilder=fake_mibbuilder_factory))
...
def test_write_schema_merge_extracted_enums(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
...
def test_detect_inherited_indexes_with_inheritance(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_extract_type_info_with_constraints_detailed(monkeypatch: pytest.MonkeyPatch) -> None:
