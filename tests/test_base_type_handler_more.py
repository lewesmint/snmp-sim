...
def test_resolve_to_base_type_fallback_logs(caplog: pytest.LogCaptureFixture, handler: BaseTypeHandler) -> None:
...
def test_create_pysnmp_value_handles_type_class_exception(monkeypatch: pytest.MonkeyPatch, handler: BaseTypeHandler, caplog: pytest.LogCaptureFixture) -> None:
    class BadCls:
...
def test_create_pysnmp_value_fallback_to_rfc1902(monkeypatch: pytest.MonkeyPatch, handler: BaseTypeHandler) -> None:
    # Simulate missing MIB type and patch rfc1902
    class FakeInt:
        def __init__(self, v: Any) -> None:
            self.v = int(v)

    fake_rfc = types.SimpleNamespace(Integer32=FakeInt, OctetString=lambda x: x if isinstance(x, bytes) else x.encode('utf-8'), ObjectIdentifier=lambda x: tuple(x))
...
def test_get_pysnmp_type_class_prefers_mib_builder_then_rfc1902(monkeypatch: pytest.MonkeyPatch, handler: BaseTypeHandler) -> None:
    class MyClass:
        pass
...
def test_get_default_value_integer_no_constraints_returns_zero(handler: BaseTypeHandler) -> None:
...
def test_get_default_value_unexpected_base_type_logs_warning(caplog: pytest.LogCaptureFixture, handler: BaseTypeHandler, mocker: MockerFixture) -> None:
    ...
    assert result == 0
    assert any("Unexpected base type 'UNKNOWN_BASE_TYPE'" in record.message for record in caplog.records)


def test_create_pysnmp_value_rfc1902_fallback_exception(caplog: pytest.LogCaptureFixture, handler: BaseTypeHandler, mocker: MockerFixture) -> None:
...
def test_get_pysnmp_type_class_rfc1902_exception(caplog: pytest.LogCaptureFixture, handler: BaseTypeHandler) -> None:
