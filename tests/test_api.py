...
def test_validate_types_invalid(monkeypatch: Any) -> None:
    ...
    monkeypatch.setattr('app.type_registry_validator.validate_type_registry_file', lambda p: (False, ['err1'], 0))
...
def test_validate_types_valid(monkeypatch: Any) -> None:
    monkeypatch.setattr('app.type_registry_validator.validate_type_registry_file', lambda p: (True, [], 42))
    r = client.get('/validate-types')
    assert r.status_code == 200
    body = r.json()
