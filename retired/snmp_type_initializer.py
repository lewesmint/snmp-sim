...
class SNMPTypeInitializer:
    _custom_initializers: Dict[
        str, Callable[[Dict[str, Any], "SNMPTypeInitializer"], Any]
    ] = {}
    ...
    def register_initializer(
        cls,
        type_name: str,
        func: Callable[[Dict[str, Any], "SNMPTypeInitializer"], Any],
    ) -> None:
    ...
    def initialize(self, col_info: Dict[str, Any]) -> Any:
        ...
        if type_class is not None:
            try:
                ...
                if self.logger:
                    self.logger.warning(
                        f"Failed to initialize {type_name} with value {value}: {e}"
                    )
                return value
        else:
            if self.logger:
