...
class _FakeEntry(_FakeSymbol):
    def __init__(
        self, name: str, oid: tuple[int, ...], indexes: list[tuple[str, str, str]]
    ) -> None:
...
def test_parse_mib_name_from_py(tmp_path: Path) -> None:
    ...
    generator = BehaviourGenerator(
        output_dir=str(tmp_path / "out"), load_default_plugins=False
    )
...
def test_get_default_value_from_type_info_plugin(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    generator = BehaviourGenerator(output_dir="/tmp", load_default_plugins=False)
    monkeypatch.setattr(
        "app.generator.get_default_value", lambda _type_info, _name: "value"
    )
    assert (
        generator._get_default_value_from_type_info(
            {"base_type": "Integer32"}, "sysDescr"
        )
        == "value"
    )
...
def test_get_default_value_from_type_info_raises(
    ...
    generator = BehaviourGenerator(output_dir="/tmp", load_default_plugins=False)
    monkeypatch.setattr(
        "app.generator.get_default_value", lambda _type_info, _name: None
    )
...
def test_extract_mib_info_inherited_indexes(
...
def test_generate_writes_json(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        generator,
        "_extract_mib_info",
        lambda _p, _n: {"sysDescr": {"oid": [1], "type": "OctetString"}},
    )
...
def test_generate_respects_existing_file(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
...
def test_generate_adds_default_row_for_table(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    output_dir = tmp_path / "out"
    generator = BehaviourGenerator(
    ...
    monkeypatch.setattr(
        generator,
        "_load_type_registry",
        lambda: {"OctetString": {"base_type": "OctetString"}},
    )
    monkeypatch.setattr(
        generator, "_get_default_value_from_type_info", lambda _t, _s: "default"
    )
...
def test_generate_extracts_index_names_from_compiled(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    output_dir = tmp_path / "out"
    generator = BehaviourGenerator(
...
def test_load_type_registry_missing(
...
def test_extract_mib_info_non_dict_symbols(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    mib_name = "TEST-MIB"
...
def test_table_without_entry_suffix(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
...
def test_get_default_value_legacy_specific_symbols() -> None:
    ...
    generator = BehaviourGenerator(output_dir="/tmp", load_default_plugins=False)
    assert (
        generator._get_default_value("DisplayString", "sysDescr")
        == "Simple Python SNMP Agent - Demo System"
    )
    assert generator._get_default_value("OID", "sysObjectID") == "1.3.6.1.4.1.99999"
    assert (
        generator._get_default_value("DisplayString", "sysContact")
        == "Admin <admin@example.com>"
    )
    assert (
        generator._get_default_value("DisplayString", "sysLocation")
        == "Development Lab"
    )
...
def test_extract_mib_info_symbol_without_methods(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
...
def test_extract_mib_info_get_name_raises_type_error(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
...
def test_extract_mib_info_none_syntax(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
...
def test_detect_inherited_indexes_exception_handling(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_table_with_no_entry_info(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
...
def test_table_entry_already_has_indexes(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    ...
    class _MockBuilder:
        ...
        def load_modules(self, _name: str) -> None:
            ...
            raise AssertionError(
                "load_modules should not be called when indexes already exist"
            )
    ...
    monkeypatch.setattr(
        generator, "_get_default_value_from_type_info", lambda _t, _s: 1
    )
...
class TestGeneratorErrorHandling:
    ...
    def test_generate_with_missing_entry_info(
    ...
    def test_generate_table_with_no_columns(
        self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
    ) -> None:
    ...
    def test_mibbuilder_exception_during_index_extraction(
        self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
    ) -> None:
    ...
    def test_generate_symbol_info_not_dict(
        self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
    ) -> None:
    ...
    def test_generate_symbol_is_table_row_not_table(
        self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
    ) -> None:
    ...
    def test_generate_table_without_type_key(
        self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
    ) -> None:
    ...
    def test_generate_rows_not_list_or_missing(
        self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
    ) -> None:
    ...
    def test_extract_type_info_constraint_with_values(
    ...
    def test_extract_type_info_constraint_no_values_attr(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
    ...
    def test_generate_handles_missing_type_key(
        self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
    ) -> None:
        ...
        # Manually set type to MibTableRow (matches condition but 'type' key
        # will be missing initially).
    ...
    def test_generate_entry_not_in_info_dict(
        self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
    ) -> None:
    ...
    def test_generate_entry_has_existing_indexes(
        self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
    ) -> None:
    ...
    def test_generate_entry_object_missing_get_index_names(
        self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
    ) -> None:
    ...
    def test_init_loads_default_plugins(
        self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
    ) -> None:
        ...
        gen = BehaviourGenerator(
            output_dir=str(tmp_path / "out"), load_default_plugins=True
        )

        # Verify it was called
        assert len(loaded_plugins) > 0

    def test_init_skips_plugins_when_disabled(
        self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
    ) -> None:
