...
def create_table_instances(
    mib_builder: builder.MibBuilder, table_name: str, table_data: Dict[str, Any]
) -> List[Tuple[str, Any]]:
    """
    ...
    for row_idx, row_data in enumerate(rows, start=1):
        ...
        for col_idx, (col_name, col_value) in enumerate(row_data.items(), start=1):
            ...
            syntax_value = convert_value_to_pysnmp_type(
                mib_builder, col_value, col_name
            )
...
def convert_value_to_pysnmp_type(
    mib_builder: builder.MibBuilder, value: Any, column_name: str
) -> Any:
    """
...
def setup_tables_in_agent(
    snmpEngine: "SnmpEngine", mib_jsons: Dict[str, Dict[str, Any]]
) -> None:
    """
    ...
    for mib_name, mib_json in mib_jsons.items():
        ...
        for table_name, table_info in tables.items():
            try:
                instances = create_table_instances(mibBuilder, table_name, table_info)

                # Export instances
                mibBuilder.export_symbols(mib_name, *instances)
...
if __name__ == "__main__":
    ...
    instances = create_table_instances(mibBuilder, "systemTable", EXAMPLE_TABLE_DATA)
