...
class SNMPAgentProcess:
    ...
    def __init__(
        self, host: str = "127.0.0.1", port: int = 11161, startup_delay: float = 15.0
    ):
        self.host = host
        self.port = port
...
class TestSNMPAgentIntegration:
    ...
    def test_snmpget_sysdescr(self, agent: SNMPAgentProcess) -> None:
        ...
        assert (
            "STRING" in result or "OCTET STRING" in result
        ), f"Unexpected type in result: {result}"
    ...
    def test_snmpget_sysuptime(self, agent: SNMPAgentProcess) -> None:
        ...
        assert (
            "Timeticks" in result or "INTEGER" in result
        ), f"Unexpected type in result: {result}"
    ...
    def test_verify_behaviour_json_matches_responses(
        self, agent: SNMPAgentProcess
    ) -> None:
    ...
    def test_multiple_snmpget_requests(self, agent: SNMPAgentProcess) -> None:
        ...
        assert (
            len(results) >= 2
        ), f"Only got {len(results)} successful responses out of {len(oids)}"
        ...
        for oid, result in results:
            print(
                f"{oid}: {result[:80]}..." if len(result) > 80 else f"{oid}: {result}"
            )
...
class TestSNMPToolsAvailable:
    ...
    def test_snmp_tools_installed(self) -> None:
        ...
        result = subprocess.run(
            ["snmpget", "--version"], capture_output=True, text=True
        )
        assert result.returncode == 0, "snmpget not available"

        # Check snmpwalk
        result = subprocess.run(
            ["snmpwalk", "--version"], capture_output=True, text=True
        )
