...
result = get_sync(engine1, auth1, address, [ObjectType(make_oid('1.3.6.1.4.1.99999.1.1.0'))], timeout=1.0, retries=1)
...
try:
    result = get_next_sync(engine2, auth2, address, [ObjectType(make_oid('1.3.6.1.4.1.99999.1.1.0'))], timeout=0.5, retries=1)
    for var_bind in result:
        print(f'✓ {var_bind}')
