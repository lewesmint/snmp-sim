...
from async_wrapper import get_sync, get_next_sync, make_oid, SnmpSyncError, SyncSnmpClient
...
def test_walk_enterprise_mib() -> None:
    ...
    try:
        while iteration < max_iterations:
            ...
            try:
                result = get_next_sync(engine, auth, address, [current_oid], timeout=1.0, retries=1)
                ...
                for var_bind in result:
                    ...
                    if not oid_str.startswith("1.3.6.1.4.1.99999"):
                        print(f"✓ WALK complete - walked past enterprise root after {len(results)} OIDs")
                        return
