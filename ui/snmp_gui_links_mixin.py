...
class SNMPGuiLinksMixin:
    ...
    def _parse_link_parts(self, text: str) -> list[str]:
        lines = [line.strip() for line in text.split("\n") if line.strip()]
        parts: list[str] = []
        for line in lines:
            if "," in line:
    ...
    def _resolve_link_parts(self, parts: list[str]) -> list[dict[str, Any]]:
        resolved_endpoints: list[dict[str, Any]] = []
        metadata = self.oid_metadata or {}
        for part in parts:
            matched = next(
                ((oid, meta) for oid, meta in metadata.items() if meta.get("name", "") == part),
                None,
            )
            if matched is None:
    ...
    def _build_verify_label_state(
        self,
        resolved_endpoints: list[dict[str, Any]],
    ) -> tuple[str, tuple[str, str]]:
        if len(resolved_endpoints) < 2:
            return "⚠ Need at least 2 OIDs to create a link", ("orange", "orange")
        ...
        if columns and scalars:
            return "⚠ ERROR: Cannot mix table columns and scalars", ("red", "red")
    ...
    def _rebuild_link_base_selection(
        self,
        *,
        base_radio_frame: ctk.CTkFrame,
    ...
    def _build_link_endpoints_payload(
        self,
        *,
        parts: list[str],
        base_index: int,
        ...
        for i, part in enumerate(parts):
            ...
            if endpoint is None:
                endpoint = {"table_oid": None, "column": part}
    ...
    def _open_link_dialog(self, link: dict[str, Any] | None) -> None:
        ...
        def _on_endpoints_change(*_args: Any) -> None:
            ...
            if not text:
                ...
                self._rebuild_link_base_selection(
                    base_radio_frame=base_radio_frame,
            ...
            parts = self._parse_link_parts(text)
            resolved_endpoints = self._resolve_link_parts(parts)
            self._rebuild_link_base_selection(
                base_radio_frame=base_radio_frame,
                base_radios=base_radios,
                base_var=base_var,
            ...
            verify_text, verify_color = self._build_verify_label_state(resolved_endpoints)
            verify_label.configure(text=verify_text, text_color=verify_color)
        ...
        def _save() -> None:
            ...
            endpoints = self._build_link_endpoints_payload(parts=parts, base_index=base_index)
