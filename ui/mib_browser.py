...
class MIBBrowserWindow:
    ...
    def _setup_browser_tab(self) -> None:
        ...
        expand_btn = ctk.CTkButton(toolbar_frame, text="Expand All", command=self._expand_all, width=100)
        expand_btn.pack(side="left", padx=(0, 6))
        
        collapse_btn = ctk.CTkButton(toolbar_frame, text="Collapse All", command=self._collapse_all, width=100)
    ...
    def _setup_mib_manager_tab(self) -> None:
        ...
        browse_btn = ctk.CTkButton(button_frame, text="📁 Browse MIB Files", command=self._browse_mib_files, width=150)
        browse_btn.pack(side="left", padx=5)
        
        load_mib_btn = ctk.CTkButton(button_frame, text="✓ Load Selected", command=self._load_selected_mib, width=130)
        load_mib_btn.pack(side="left", padx=5)
        
        check_deps_btn = ctk.CTkButton(button_frame, text="🔍 Check Dependencies", command=self._show_mib_dependencies, width=150)
        check_deps_btn.pack(side="left", padx=5)
        
        remove_btn = ctk.CTkButton(button_frame, text="✗ Remove Selected", command=self._remove_cached_mib, width=130)
        remove_btn.pack(side="left", padx=5)
        
        refresh_btn = ctk.CTkButton(button_frame, text="🔄 Refresh List", command=self._refresh_cached_mibs, width=120)
                ...
                if missing_deps:
                    ...
                    if failed_deps:
                        ...
                        try:
                            ...
                            self.logger.log(f"Loaded MIB: {mib_name} (with {len(resolved_deps)} dependencies)", "INFO")
                        except Exception as e:
    ...
    def _show_loaded_mibs(self) -> None:
        ...
        if not self.loaded_mibs:
            ...
            messagebox.showinfo("Loaded MIBs", f"Currently loaded MIBs:\n\n{mib_list}", parent=self.window)
    ...
    def _load_selected_mib(self) -> None:
        ...
        selected_files = [
            Path(file_path) for file_path, var in self.cached_mib_checkbuttons.items()
            if var.get()
        ]
        ...
        for mib_file in selected_files:
            ...
            if missing_deps:
                ...
                failed_mibs[mib_name] = f"Missing {len(missing_deps)} required dependency(ies):\n  • {missing_list}"
                self.logger.log(f"Cannot load {mib_name}: missing dependencies {missing_deps}", "ERROR")
            else:
                loaded, failed = self.load_mib([mib_name])
                if loaded:
                    loaded_count += 1
                else:
    ...
    def _remove_cached_mib(self) -> None:
        ...
        selected_files = [
            file_path for file_path, var in self.cached_mib_checkbuttons.items()
            if var.get()
        ]
    ...
    def _snmp_get(self) -> None:
        ...
        try:
            ...
            if errorIndication:
                ...
                messagebox.showerror("SNMP GET Error", str(errorIndication), parent=self.window)
                return
            elif errorStatus:
                self.status_var.set(f"Error: {errorStatus.prettyPrint()}")
                self.logger.log(f"MIB Browser GET error: {errorStatus.prettyPrint()}", "ERROR")
                messagebox.showerror("SNMP GET Error", errorStatus.prettyPrint(), parent=self.window)
                return
            ...
            self.logger.log(f"MIB Browser: GET {display_oid} returned {len(varBinds)} result(s)")
    ...
    def _snmp_getnext(self) -> None:
        ...
        try:
            ...
            if errorIndication:
                ...
                messagebox.showerror("SNMP GETNEXT Error", str(errorIndication), parent=self.window)
                return
            elif errorStatus:
                self.status_var.set(f"Error: {errorStatus.prettyPrint()}")
                self.logger.log(f"MIB Browser GETNEXT error: {errorStatus.prettyPrint()}", "ERROR")
                messagebox.showerror("SNMP GETNEXT Error", errorStatus.prettyPrint(), parent=self.window)
                return
            ...
            self.logger.log(f"MIB Browser: GETNEXT {display_oid} returned {len(varBinds)} result(s)")
        ...
        try:
            ...
            for errorIndication, errorStatus, errorIndex, varBinds in walk_results:
                ...
                if errorIndication:
                    ...
                    messagebox.showerror("SNMP WALK Error", str(errorIndication), parent=self.window)
                    return
                elif errorStatus:
                    self.status_var.set(f"Error: {errorStatus}")
                    self.logger.log(f"MIB Browser WALK error: {errorStatus}", "ERROR")
                    messagebox.showerror("SNMP WALK Error", str(errorStatus), parent=self.window)
                    return
    ...
    def _snmp_set(self) -> None:
        ...
        try:
            ...
            if errorIndication:
                ...
                messagebox.showerror("SNMP SET Error", str(errorIndication), parent=self.window)
                return
            elif errorStatus:
                self.status_var.set(f"Error: {errorStatus.prettyPrint()}")
                self.logger.log(f"MIB Browser SET error: {errorStatus.prettyPrint()}", "ERROR")
                messagebox.showerror("SNMP SET Error", errorStatus.prettyPrint(), parent=self.window)
                return
