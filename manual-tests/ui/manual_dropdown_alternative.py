...
class AlternativeDropdownTest:
    def __init__(self):
        ...
        # Status
        self.status = tk.Label(self.root, text="Click on Column 2 cells to edit", bg="lightgray", fg="black")
    ...
    def _show_listbox_popup(self, event, item, column, current_value, bbox):
        ...
        # Bind selection
        listbox.bind("<<ListboxSelect>>", lambda e: self._on_listbox_select(listbox, enum_values))
        listbox.bind("<Button-1>", lambda e: self._on_listbox_click(e, listbox, enum_values))
